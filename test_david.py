...
class ConfusionMatrix:
    ...
    ############################################################################################################
    ############################################################################################################
    def process_batch(self, detections, labels, stem, test_path, save_dir='.'):
        """
        ...
        for i, gc in enumerate(gt_classes):
            ...
            if n and sum(j) == 1:
                ...
                if 'FN' in mode:
                    try:
                        shutil.copy(f'{test_path}/images/{stem}.jpg', f'{save_dir}/picked_FN/{stem}.jpg')
                    except:
                        print(f'{stem} no .jpg')
        ...
        if n:
            for i, dc in enumerate(detection_classes):
                if not any(m1 == i):
                    ...
                    if 'FP' in mode:
                        try:
                            shutil.copy(f'{test_path}/images/{stem}.jpg', f'{save_dir}/picked_FP/{stem}.jpg')
                        except:
                            print(f'{stem} no .jpg')
...
def test(data,
    ...
    if training:  # called by train.py
        ...
        ############################################################################################################
        if 'FP' in mode:
            (save_dir / ('picked_FP')).mkdir(parents=True, exist_ok=True)
        if 'FN' in mode:
            (save_dir / ('picked_FN')).mkdir(parents=True, exist_ok=True)
        ...
        for si, pred in enumerate(out):
            ...
            if nl:
                ...
                if plots:
                    ...
                    ############################################################################################################
                    test_path = data[task]
                    confusion_matrix.process_batch(predn, torch.cat((labels[:, 0:1], tbox), 1), path.stem, test_path, save_dir)
